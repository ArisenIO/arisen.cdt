...
using arisen::binary_extension;
using arisen::datastream;
using arisen::fixed_bytes;
using arisen::ignore;
using arisen::ignore_wrapper;
using arisen::pack;
using arisen::pack_size;
...
struct be_test {
   ...
   be_test(int i) : val{i} {}
   int val;
   RSNLIB_SERIALIZE( be_test, (val) )
};

// Definitions in `arisen.cdt/libraries/arisen/datastream.hpp`
ARISEN_TEST_BEGIN(datastream_test)
...
// Definitions in `arisen.cdt/libraries/arisen/datastream.hpp`
ARISEN_TEST_BEGIN(datastream_specialization_test)
   static constexpr uint16_t buffer_size{256};
   char datastream_buffer[buffer_size]{}; // Buffer for the datastream to point to
   char buffer[buffer_size]; // Buffer to compare `datastream_buffer` with
...
// Definitions in `arisen.cdt/libraries/arisen/datastream.hpp`
ARISEN_TEST_BEGIN(datastream_stream_test)
   static constexpr uint16_t buffer_size{256};
   char datastream_buffer[buffer_size]; // Buffer for the datastream to point to
...
// Definitions in `arisen.cdt/libraries/arisen/datastream.hpp`
ARISEN_TEST_BEGIN(misc_datastream_test)
   // ---------------------------
...
int main(int argc, char* argv[]) {
   ...
   ARISEN_TEST(datastream_test);
   ARISEN_TEST(datastream_specialization_test);
   ARISEN_TEST(datastream_stream_test);
   ARISEN_TEST(misc_datastream_test);
   return has_failed();
}
