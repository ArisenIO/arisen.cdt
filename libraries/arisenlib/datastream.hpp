...
#warning "<arisenlib/datastream.hpp> is deprecated use <arisen/datastream.hpp>"
   ...
   public:
      ...
      */
      inline bool read( char* d, size_t s ) {
        arisen::check( size_t(_end - _pos) >= (size_t)s, "read" );
        memcpy( d, _pos, s );
        _pos += s;
        return true;
      }
      ...
      */
      inline bool write( const char* d, size_t s ) {
        arisen::check( _end - _pos >= (int32_t)s, "write" );
...
void deserialize(datastream<Stream>& ds, std::variant<Ts...>& var, int i) {
   if constexpr (I < std::variant_size_v<std::variant<Ts...>>) {
      ...
      arisen::check(false, "invalid variant index");
   }
}
...
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const arisen::symbol sym) {
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, arisen::symbol& sym) {
  uint64_t raw = 0;
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const ::arisen::ignore_wrapper<T>& val) {
  ds << val.value;
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const ::arisen::ignore<T>& val) {
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, ::arisen::ignore<T>) {
  return ds;
}
 ...
 *
 *  @brief Serialize an arisen::public_key
 *  @param ds - The stream to write
 *  @param pubkey - The value to serialize
 *  @tparam Stream - Type of datastream buffer
 *  @return datastream<Stream>& - Reference to the datastream
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const arisen::public_key& pubkey) {
   ds << pubkey.type;
 ...
 *
 *  @brief Deserialize an arisen::public_key
 *  @param ds - The stream to read
 *  @param pubkey - The destination for deserialized value
 *  @tparam Stream - Type of datastream buffer
 *  @return datastream<Stream>& - Reference to the datastream
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, arisen::public_key& pubkey) {
   ds >> pubkey.type;
 ...
 *
 *  @brief Serialize an arisen::signature
 *  @param ds - The stream to write
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const arisen::signature& sig) {
   ds << sig.type;
 ...
 *
 *  @brief Deserialize an arisen::signature
 *  @param ds - The stream to read
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, arisen::signature& sig) {
   ds >> sig.type;
...
DataStream& operator >> ( DataStream& ds, T (&v)[N] ) {
   unsigned_int s;
   ds >> s;
   arisen::check( N == s.value, "T[] size and unpacked size don't match");
   for( uint32_t i = 0; i < N; ++i )
      ds >> v[i];
   return ds;
}
