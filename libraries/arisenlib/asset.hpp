...
#warning "<arisenlib/asset.hpp> is deprecated use <arisen/asset.hpp>"
...
namespace arisen {
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         arisen::check( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         arisen::check( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         arisen::check( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         arisen::check( -max_amount <= amount, "subtraction underflow" );
         arisen::check( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         arisen::check( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         arisen::check( -max_amount <= amount, "addition underflow" );
         arisen::check( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ...
         arisen::check( tmp <= max_amount, "multiplication overflow" );
         arisen::check( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         arisen::check( a != 0, "divide by zero" );
         arisen::check( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         arisen::check( b.amount != 0, "divide by zero" );
         arisen::check( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
   ...
   */
   struct extended_asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         arisen::check( a.contract == b.contract, "type mismatch" );
      ...
      RSNLIB_SERIALIZE( extended_asset, (quantity)(contract) )
   };
