...
#include "core/arisen/powers.hpp"
#include "contracts/arisen/system.hpp"
#include "contracts/arisen/privileged.hpp"
...
namespace arisen {
   ...
   void get_blockchain_parameters(arisen::blockchain_parameters& params) {
      ...
      arisen::check( size <= sizeof(buf), "buffer is too small" );
   ...
   std::optional<uint64_t> set_proposed_producers( const std::vector<producer_key>& prods ) {
      auto packed_prods = arisen::pack( prods );
