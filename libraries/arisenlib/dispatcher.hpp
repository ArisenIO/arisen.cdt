...
#warning "<arisenlib/dispatcher.hpp> is deprecated use <arisen/dispatcher.hpp>"
...
namespace arisen {
 ...
 // Helper macro for ARISEN_DISPATCH_INTERNAL
 #define ARISEN_DISPATCH_INTERNAL( r, OP, elem ) \
    case arisen::name( BOOST_PP_STRINGIZE(elem) ).value: \
       arisen::execute_action( arisen::name(receiver), arisen::name(code), &OP::elem ); \
       break;

 // Helper macro for ARISEN_DISPATCH
 #define ARISEN_DISPATCH_HELPER( TYPE,  MEMBERS ) \
    BOOST_PP_SEQ_FOR_EACH( ARISEN_DISPATCH_INTERNAL, TYPE, MEMBERS )

/**
 * @addtogroup dispatcher
 * Convenient macro to create contract apply handler
 *
 * @note To be able to use this macro, the contract needs to be derived from arisen::contract
