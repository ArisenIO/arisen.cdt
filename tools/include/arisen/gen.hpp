...
struct generation_utils {
      ...
      auto check = [&](const clang::Type* pt) {
        if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
         if (auto rt = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
            return rt->getDecl()->isArisenIgnore();
      ...
      auto get = [&](const clang::Type* pt) {
         if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
            if (auto decl = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
               return decl->getDecl()->isArisenIgnore() ? tst->getArg(0).getAsType() : type;
         return type;
      };
   ...
   static inline bool has_arisen_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->hasArisenRicardian();
   }
   static inline bool has_arisen_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->hasArisenRicardian();
   }

   static inline std::string get_arisen_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->getArisenRicardianAttr()->getName();
   }
   static inline std::string get_arisen_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->getArisenRicardianAttr()->getName();
   }

   static inline std::string get_action_name( const clang::CXXMethodDecl* decl ) {
      std::string action_name = "";
      auto tmp = decl->getArisenActionAttr()->getName();
      if (!tmp.empty())
         return tmp;
   ...
   static inline std::string get_notify_pair( const clang::CXXMethodDecl* decl ) {
      ...
      auto tmp = decl->getArisenNotifyAttr()->getName();
      return tmp;
   }
   ...
   static inline bool is_arisen_contract( const clang::CXXMethodDecl* decl, const std::string& cn ) {
      std::string name = "";
      if (decl->isArisenContract())
         name = decl->getArisenContractAttr()->getName();
      else if (decl->getParent()->isArisenContract())
         name = decl->getParent()->getArisenContractAttr()->getName();
      if (name.empty()) {
   ...
   static inline bool is_arisen_contract( const clang::CXXRecordDecl* decl, const std::string& cn ) {
      std::string name = "";
      auto pd = llvm::dyn_cast<clang::CXXRecordDecl>(decl->getParent());
      if (decl->isArisenContract()) {
         auto nm = decl->getArisenContractAttr()->getName().str();
         name = nm.empty() ? decl->getName().str() : nm;
      }
      else if (pd && pd->isArisenContract()) {
         auto nm = pd->getArisenContractAttr()->getName().str();
