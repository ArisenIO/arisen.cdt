...
#include <arisen/gen.hpp>

#include <arisen/utils.hpp>
#include <arisen/whereami/whereami.hpp>
#include <arisen/abi.hpp>
...
using namespace arisen::cdt;

namespace arisen { namespace cdt {
   ...
   struct codegen_exception : public std::exception {
      virtual const char* what() const throw() {
         return "arisen.codegen fatal error";
      }
   };
   ...
   // remove after v1.7.0
   bool has_arisenlib = false;

   class arisen_ppcallbacks : public PPCallbacks {
      public:
         arisen_ppcallbacks(SourceManager& sm, std::string file) : sources(sm), fn(file) {}
      protected:
         virtual void InclusionDirective(
            ...
            if ( file_name.find("arisenlib") != StringRef::npos )
               has_arisenlib = true;
         }
   ...
   class arisen_codegen_visitor : public RecursiveASTVisitor<arisen_codegen_visitor>, public generation_utils {
      private:
      ...
      public:
         ...
         explicit arisen_codegen_visitor(CompilerInstance *CI)
         ...
         bool is_datastream(const QualType& qt) {
            ...
            auto ds_re    = std::regex("(((class arisen::)?datastream<[a-zA-Z]+[a-zA-Z0-9]*.*>)|(DataStream)) &");
         ...
         */
         ...
         void create_dispatch(const std::string& attr, const std::string& func_name, F&& get_str, CXXMethodDecl* decl) {
            ...
            if (cg.is_arisen_contract(decl, cg.contract_name)) {
               if (has_arisenlib) {
                  ss << "\n\n#include <arisenlib/datastream.hpp>\n";
                  ss << "#include <arisenlib/name.hpp>\n";
               } else {
                  ss << "\n\n#include <arisen/datastream.hpp>\n";
                  ss << "#include <arisen/name.hpp>\n";
               }
               ss << "extern \"C\" {\n";
               ...
               ss << "}\n";
               ss << "arisen::datastream<const char*> ds{(char*)buff, as};\n";
               int i=0;
               ...
               ss << decl->getParent()->getQualifiedNameAsString() << "{arisen::name{r},arisen::name{c},ds}." << decl->getNameAsString() << "(";
         ...
         void create_action_dispatch(CXXMethodDecl* decl) {
            ...
            create_dispatch("arisen_wasm_action", "__arisen_action_", func, decl);
         }

         void create_notify_dispatch(CXXMethodDecl* decl) {
            auto func = [](CXXMethodDecl* d) { return generation_utils::get_notify_pair(d); };
            create_dispatch("arisen_wasm_notify", "__arisen_notify_", func, decl);
         }
         ...
         virtual bool VisitCXXMethodDecl(CXXMethodDecl* decl) {
            ...
            if (decl->isArisenAction()) {
               name = generation_utils::get_action_name(decl);
               validate_name(name, [&]() {emitError(*ci, decl->getLocation(), "action not a valid arisen name");});
      ...
      class arisen_codegen_consumer : public ASTConsumer {
      private:
         arisen_codegen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit arisen_codegen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new arisen_codegen_visitor(CI)), main_file(file), ci(CI) { }
         ...
         virtual void HandleTranslationUnit(ASTContext &Context) {
            ...
            if (main_fe) {
               ...
               try {
                  ...
                  std::stringstream ss;
                  ss << "extern \"C\" {\n";
                  ss << "void arisen_assert_code(uint32_t, uint64_t);";
                  ss << "\t__attribute__((weak, arisen_wasm_entry, arisen_wasm_abi(";
                  std::string abi = cg.abi;
                  ss << "\"" << _quoted(abi) << "\"";
                  ss << ")))\n";
                  ss << "\tvoid __insert_arisen_abi(unsigned long long r, unsigned long long c, unsigned long long a){";
                  ss << "arisen_assert_code(false, 1);";
                  ss << "}\n";
                  ss << "}";
      ...
      class arisen_codegen_frontend_action : public ASTFrontendAction {
      public:
         virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) {
            CI.getPreprocessor().addPPCallbacks(_make_unique<arisen_ppcallbacks>(CI.getSourceManager(), file.str()));
            return _make_unique<arisen_codegen_consumer>(&CI, file);
         }
   };
